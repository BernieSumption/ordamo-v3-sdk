...
var OrdamoSDK = (function () {
    ...
    OrdamoSDK.prototype._initialiseDevelopmentData = function () {
        var _this = this;
        if (this._options.contentOverride) {
            setTimeout(function () { return _this._receiveDevelopmentContent(_this._options.contentOverride); }, 1);
        }
        else {
            var DEFAULT_CONTENT_FILE_1 = "default-content.json";
            var xhr_1 = new XMLHttpRequest();
            xhr_1.open("GET", DEFAULT_CONTENT_FILE_1, true);
            xhr_1.onreadystatechange = function () {
                if (xhr_1.readyState === 4) {
                    if (xhr_1.status === 200) {
                        var content = void 0;
                        try {
                            content = JSON.parse(xhr_1.responseText);
                        }
                        catch (e) {
                            console.error(DEFAULT_CONTENT_FILE_1 + " is not a valid JSON file, check the console for more info");
                            console.error(e);
                            logNotice("This content is not JSON", xhr_1.responseText);
                        }
                        if (content) {
                            _this._receiveDevelopmentContent(content);
                        ...
                        console.error("Failed to load \"" + DEFAULT_CONTENT_FILE_1 + "\", is the development server running (npm start)");
                    }
                }
            };
            xhr_1.send();
    ...
    OrdamoSDK.prototype._receiveDevelopmentContent = function (content) {
